...
// TODO: 'msg' below isn't getting the expand()ed string.  WTF???
		...
		// Execute the method for each item in the list
		// Order isn't important.  Does not return any results (since they'd be unordered).
		// If any of the items fail, we'll fail the outer promise.
		//
		// Pass a number for `limit` to limit the number of things happening at once,
		//	otherwise all will start immediately when this task begins.
		parallelize : function(message, list, method, limit) {
			var taskList = this;
			var parallelPromise = $.Deferred();

			var remaining = taskList.remaining = [];
			var working = taskList.working = [];

			function startMore() {
				// bail if we've cancelled the entire task list
				if (taskList.groupState() !== "pending") return;

				while (remaining.length > 0 && working.length < limit) {
					try {
						// wrap this in a function to create 'success' and 'failure' handlers for each item.
						(function() {
							var item = remaining.shift();
							var result = method.call(taskList.scope, item);

							// if we got a promise back, add it to the `working` queue
							if (TaskList.isAPromise(result) && result.state() === "pending") {
								var _success = function() {
//	console.info("_success for item ",item);
									working.remove(itemTask);
									startMore();
								}
								var _failure = function(error) {
//	console.info("_failure for item ",item);
									working.remove(itemTask);
									reject(error);
								}

								var itemTask = result.then(_success, _failure);
								working.append(itemTask);
//	console.info("queueing ",item,"  working:",working.length, " remaining: ",remaining.length);
							}
							else {
//	console.log("item finished already:", item);
							}
						})();
					} catch (error) {
//console.info("error calling ",method,": ",error);
						reject(error);
					}
				}
			...
			function resolve() {
//console.warn(message+" RESOLVED");
				parallelPromise.resolveWith(taskList.scope);
			}

			function reject(error) {
//console.error(message+" REJECTED:", error);
				parallelPromise.rejectWith(taskList.scope, [error]);
			}

			function start() {
				// default to running everything at once
				if (!limit) limit = list.length;
				// if list is a function, call that to get the actual list
				if (typeof list === "function") list = list.call(taskList.scope);

console.warn("list: ",list.length," limit:",limit);

				// duplicate the list so we can pull things out of it
				remaining.addList(list);
