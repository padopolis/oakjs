...
function(Singleton, Widget, Panel, Page, Modal)
{
	...
	new Singleton("UI", {
		...
		appName : "YOU_MUST_SET_UI.appName!",
		...
		// Initialize the entire UI.  Called from the package handler callback in your .html file.
		//	Most significanly, this:
		//		- parses widgets out of the page HTML and starts loading any `load="now"` widgets.
		//		- sets up any global UI events via	`setupUIEvents()`
		//		- authenticates the user via 		`authenticateUser()`
		//		- loads startup data via			`loadStartupData()`
		//	Once those complete, we call `UI.onReady()`
		//  Returns a promise for the sequence above.
		//
		//	NOTE: You should not override this, implement the above functions instead.
		initialize : function() {
			if (!UI.domReady) {
			...
			// Left and right sidebar stuff, visibility maintained in a cookie. Used by publisher.
			// TODO: reconsider this pattern -- move into specific UIs.
			...
			// Restore zoom scale on last load.
			...
			var promise = 	UI.authenticateUser()
			//				2) Load startup data
							.then(UI.bind("loadStartupData"))
			//				3) Call onReady
							.then(UI.bind("onReady"), UI.bind("onReadyError"));
		...
		// Authenticate the user if there is one.
		// App startup will not proceed until this has been completed.
		// MUST return a promise to be resolved when the user is logged in.
		authenticateUser : function() {
			// By default, we don't do authentication, override to authenticate in your app.
		...
		prompt : function(message, defaultValue, okButtonTitle, cancelButtonTitle) {
			if (!UI.Prompt) {
				UI.Prompt = new Panel({
					...
					keyMap : [
						{ keys:"RETURN", whenFocused:true, enabled:true, handler:"onOK", hint:"OK"},
						{ keys:"NUM-ENTER", whenFocused:true, enabled:true, handler:"onOK", hint:" "},
					],
