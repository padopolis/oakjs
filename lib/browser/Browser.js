...
// TODO: BREAKS GLOBAL ENCAPSULATION!
		...
		is : {
			...
			chrome			: userAgent.contains("chrome") || userAgent.contains("atom-shell"),
			...
			atomShell		: userAgent.contains("atom-shell"),
		}
	};
	Browser.is.ios = (Browser.is.ipad || Browser.is.iphone);
	Browser.is.safari = (userAgent.contains("safari") && !Browser.is.chrome && !Browser.is.phantom && !Browser.is.atomShell);
	...
	//
	if (global.JSON == null) {
		document.write("<script src='//cdnjs.cloudflare.com/ajax/libs/json2/20130526/json2.min.js'></script>");
	}
